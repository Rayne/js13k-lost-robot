...
/**
 * This class is largely based upon `php-svg-lib`'s `Svg\Tag\Path` class.
 *
...
class Path
{
    ...
    /**
     * @param string $pathAttribute `d` path.
     * @return array[][] Global polygons.
     */
    public function getGlobalPolygons($pathAttribute)
    {
        $commands = [];
        preg_match_all('/([MZLHVCSQTAmzlhvcsqta])([eE ,\-.\d]+)*/', $pathAttribute, $commands, PREG_SET_ORDER);
        ...
        $surface = new SurfacePointCollector();
        ...
        return $surface->getGlobalPolygons();
    }

    function drawArc(SurfacePointCollector $surface, $fx, $fy, $coords)
    {
        $rx = $coords[0];
        $ry = $coords[1];
        $rot = $coords[2];
